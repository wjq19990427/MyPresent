...
`v0.6.1` · 下一里程碑：Phase 7 前后端重构（FastAPI + Next.js）
...
- **task-60**：详情页三段化（文本→分类→元数据）；AI 标签扩展至四维度，按需提出新标签；情绪描述移至元数据段
- **task-59**：AI 分析拆分为内容与标签两路；内容字段建议内联显示在输入框下方，逐条采纳
- **task-57**：AI 分析面板改为内联 toggle；归档台新建分组 expander 同步内联化
- **task-58**：AI 标签建议一键填入话题多选框；toast 提示；AI 新增标签橙色区分；保存时才写库
- **task-55**：系统页新增修改密码面板；用户看板升级卡片式布局
- **task-56**：修复待办树形层级缩进失效：列比例分割 + CSS 竖线/L 形连接线
