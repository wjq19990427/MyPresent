...
1. **主动读代码**：开始实现前，读涉及文件的现有代码，理解已有模式和约定——实现风格应与项目保持一致，不要凭空发明新模式
2. **Bug 自行诊断**：任务卡只描述症状和预期行为时，你负责读代码找根因、设计修复方案，不要等架构师给具体答案
3. **接口约定是边界，How 是你的**：任务卡给出函数签名和行为要求，实现细节（算法、数据结构、代码组织）由你决定
4. **最小改动**：只完成任务卡列出的事，不做计划外重构、抽象、风格调整、防御性扩展
5. **L2 同步**：若任务确实改动了公开 API、返回结构、副作用或调用语义，必须在同一提交中更新对应 `docs/api/*.md`
6. **验收**：实现完成后至少跑通三件事：
...
8. **契约冲突就停手**：发现任务卡与 L2 契约或现有代码矛盾，立即写 `BLOCKED: <冲突点>`，等架构师确认，**不要**自行绕开
