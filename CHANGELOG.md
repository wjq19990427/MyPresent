...
### 新功能
- 规划台待办升级为无限递归树形结构：可添加子级、勾选级联完成子树并移入已完成事务、已移入节点置灰禁交互、父节点显示完成百分比（task-53）
- 管理员用户管理面板：cloud 模式下 admin 账户可在系统页新增用户、查看用户名列表（task-51）
- 规划台待办完成弹窗补充终止时间，小时/分钟分开选，自动计算时长，终止时间可选（task-52）
- 规划台已完成事务支持编辑，可修改描述、分类、时间段和时长（task-54）
- 系统页新增修改密码面板（cloud 已登录用户均可修改自己的密码）；管理员用户看板升级为卡片式布局，显示总用户数与管理员标识（task-55）
- 修复待办树形层级缩进完全失效问题：改用列比例分割 + CSS 竖线/L 形连接线实现真实层级偏移（task-56）
- AI 分析面板改为内联 toggle 展开块，新建分组同步内联化，消除"额外窗口"感（task-57）
- AI 标签建议一键直接填入话题多选框，toast 提示，AI 新增标签橙色区分，保存时才写库（task-58）
- AI 分析拆分为内容与标签两路：内容 AI 只生成标题/摘要/感受/原因/情绪描述，建议内联显示在对应字段正下方，逐条采纳（task-59）
- 详情页三段化布局（文本→分类标签→内容元数据）；AI 标签扩展至全四维度（领域/视角/话题/情绪），按需生成新标签；情绪描述移至第三段（task-60）
